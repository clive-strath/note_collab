...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Empty body or parse error
  }

  // Verify user has access to this room (note)
  if (room) {
    const { data: note } = await adminSupabase
      .from('notes').select('owner_id, is_locked, folder_id').eq('id', room).single()
    ...
    if (note && note.owner_id !== user.id) {
      ...
      let hasAccess = !!perm
      if (!perm && note.folder_id) {
         const { data: folderPerm } = await adminSupabase
           .from('folder_permissions').select('role').eq('folder_id', note.folder_id).eq('user_id', user.id).single()
